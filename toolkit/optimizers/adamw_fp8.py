    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                s_m = state.get('scale_m', torch.tensor(1.0, device=p.device))
                s_v = state.get('scale_v', torch.tensor(1.0, device=p.device))

                exp_avg = self._from_fp8_scaled(state['exp_avg'], s_m, torch.float32)
                exp_avg_sq = self._from_fp8_scaled(state['exp_avg_sq'], s_v, torch.float32)

                if group.get('amsgrad', False):
                    s_max_v = state.get('scale_max_v', torch.tensor(1.0, device=p.device))
                    max_exp_avg_sq = self._from_fp8_scaled(state['max_exp_avg_sq'], s_max_v, torch.float32)
